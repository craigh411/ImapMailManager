...
class ImapMailboxService
{
	...
	/**
	 * Moves the messages from the current folder to the given folder
	 * @param $messageList
	 * @param $folder
	 * @return bool
	 */
	public function moveMessages($messageList, $folder) {
		$folder = $this->getFolderByAlias($folder);
		if($this->imap->moveMessages($messageList, $folder)) {
			return $this->imap->deleteMessages();
		}
	...
	/**
	 * Returns the number of messages for the current folder
	 * @param $folder
	 * @return int
	 */
	public function getMessageCount($folder) {
		if($folder === $this->getMailbox()->getFolder()) {
			return $this->imap->getMessageCount();
		}
		$currentFolder = $this->getMailbox()->getFolder();
